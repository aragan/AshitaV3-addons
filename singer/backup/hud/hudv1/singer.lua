...
local function echo(msg)
    ...
    local d0 = tonumber(start_delay) or 0
    local ds = tonumber(step_delay) or 0.15
    local limit = tonumber(max_lines) or #lines

    local d = d0
    for i = 1, math.min(#lines, limit) do
        echo_line_delayed(lines[i], d)
        d = d + ds
    end
end
...
local function build_steps_for_song_list(list)
    ...
    if state.ccsv then
        steps[#steps+1] = { cmd = '/ja "Clarion Call" <me>', wait = 1.0 }
        steps[#steps+1] = { cmd = '/ja "Soul Voice" <me>',   wait = 1.0 }
        actions = actions + 2
    end
